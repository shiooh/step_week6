...
N = 8192 にもなると joint_clusters() がオーバーヘッドになるので、ここを例えば各クラスターの境界部分にあるパスだけを探索する、などして改善したい。（未実装）
...
どの input でも、分割数 n に対する 巡回路長 l(n) の関数は下に凸の凸関数のようになっており、常に l(n) が凸関数であるとするなら、せいぜい分割数 10 で十分最善に達することができているといえる。
